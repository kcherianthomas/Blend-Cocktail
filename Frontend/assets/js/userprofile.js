...
var appClientId = "10apj9lf45ih1ar5klh9454qsg";
var userPoolId = "us-east-1_abfeb6rIp";
var redirectURI = "https://dbboxdskgfqfr.cloudfront.net/webcam.html";
...
async function main() {
  ...
  .then((data) => {
    ...
    // Display tokens
    console.log(parseJWTPayload(tokens.id_token).email)
    console.log(parseJWTPayload(tokens.access_token).username)
    ...
    localStorage.setItem("cocktailidtoken", JSON.stringify(parseJWTPayload(tokens.id_token)))
    localStorage.setItem("cocktailaccesstoken", JSON.stringify(parseJWTPayload(tokens.access_token)))
    localStorage.setItem("cocktailemailid", parseJWTPayload(tokens.id_token).email)
    localStorage.setItem("cocktailusername", parseJWTPayload(tokens.access_token).username)
  });
    ...
    .then((data) => {
      ...
      localStorage.setItem("cocktailuserinfo", JSON.stringify(parseJWTPayload(data)))
    });
  }}}
  main();